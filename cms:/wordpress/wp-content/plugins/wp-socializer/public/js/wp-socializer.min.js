...
return 0==e?t:t.split(" ").slice(0,e).join(" ")},l=function(t){var e={"{title}":s.ptitle,"{url}":s.purl,"{s-url}":s.psurl,"{twitter-username}":s.ptuname,"{excerpt}":a()}
for(var r in e)e.hasOwnProperty(r)&&(t=t.replace(RegExp(r,"g"),e[r]))
return t}
if(""!=s.cnt_sel)for(var i=s.cnt_sel.replace(/[\[\]<>"'\/\\=&%]/g,""),c=document.querySelectorAll(i),p=0;p<c.length;p++){var d=c[p]
d.addEventListener("mousedown",function(t){wpsr_tsb.startx=t.pageX,wpsr_tsb.starty=t.pageY}),d.addEventListener("mouseup",function(t){var e=r()
...
for(var u=tsb.querySelectorAll("a"),p=0;p<u.length;p++){var t=u[p]
t.addEventListener("click",function(t){var e=this.getAttribute("data-link")
"#"!=e&&(rep_link=l(e),wpsr_helpers.popup(rep_link,800,500)),t.preventDefault()})}}if(msb=document.querySelector(".wpsr-mobile-sb"),msb){var w=0
...
mow.length>0&&["resize","load"].forEach(function(t){window.addEventListener(t,function(){for(p=0;p<mow.length;p++){b(mow[p],window.innerWidth)}})})})
